...
class HintSystem {
  ...
  /// Returns a tuple of (fromTube, toTube) or (-1, -1) if no hint available
  static (int, int) findBestMove(GameState gameState) {
    ...
    return (possibleMoves.first.$1, possibleMoves.first.$2);
  }
  ...
  static int _evaluateMove(List<ColorTube> tubes, int fromTube, int toTube) {
    ...
    if (toTubeIsEmpty) {
      ...
      if (blockCount == 4 && tubes[fromTube].blocks.every((b) => b == fromColor)) {
      ...
      if (fromColor == toColor) {
        ...
        int transferCount = blockCount > availableSpace ? availableSpace : blockCount;
        ...
        if (tubes[fromTube].blocks.length > blockCount) {
          ...
          List<Color> remainingBlocks = tubes[fromTube].blocks.sublist(
            0, tubes[fromTube].blocks.length - blockCount);
  ...
  static int findOptimalSolution(GameState initialState) {
    ...
    while (movesMade < maxMoves && !currentState.isGameComplete) {
      (int fromTube, int toTube) = findBestMove(currentState);
      
      if (fromTube == -1 || toTube == -1) {
        break; // No more moves available
      }
      
      // Make the move
      currentState = currentState.executeMove(fromTube, toTube);
      movesMade++;
    ...
    for (int fromTube = 0; fromTube < state.tubes.length; fromTube++) {
      ...
      for (int toTube = 0; toTube < state.tubes.length; toTube++) {
        ...
        if (state.canMove(fromTube, toTube)) {
          validMoves.add((fromTube, toTube));
        }
      }
    }
